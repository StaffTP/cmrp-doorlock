...
local function OpenDoorsWithPerms() 
	...
	for i=1, #Config.DoorList do
		...
		if distance < maxDistance then
			...
			TriggerServerEvent('CMRP:UpdateVerrouillageServer', i, doorID.doorLock)	
		end
	end
end

local function ApplyDoorState(doorID)
    local closeDoor = GetClosestObjectOfType(doorID.objCoords.x, doorID.objCoords.y, doorID.objCoords.z, 1.0, doorID.objHash, false, false, false)
    FreezeEntityPosition(closeDoor, doorID.doorLock)
end


Citizen.CreateThread(function()
	TriggerServerCallback('CMRP:GetInfoPortes', function(doorInfo, count)
...
RegisterNetEvent('CMRP:UpdateVerrouillageClient')
AddEventHandler('CMRP:UpdateVerrouillageClient', function(doorID, state)
...
--> CallBack Main from NUI : used to show the cde prompt
